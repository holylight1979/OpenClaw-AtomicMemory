...
3. **Build Gate**：`requireBuildPass = true` 時 build 失敗 → 不 commit
4. **Auto-Revert**：`autoRevertOnFailure = true` 時 build 失敗 → 自動回滾
5. **Git 追蹤**：所有變更有 commit，隨時可 revert
6. **Disabled by Default**：`enabled: false` 預設，owner 需明確開啟
